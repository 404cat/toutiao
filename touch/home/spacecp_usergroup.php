...
<div class="b_p15 cl">
	<div class="bz_ug_be">
		...
		<form id="buygroupform_{$groupid}" name="buygroupform_{$groupid}" method="post" autocomplete="off" action="home.php?mod=spacecp&ac=usergroup&do=$do&groupid=$groupid"{if !empty($_GET['inajax'])} onsubmit="ajaxpost('buygroupform_{$groupid}', 'return_$_GET['handlekey']', 'return_$_GET['handlekey']', 'onerror');return false;"{/if}>
			...
			<div class="mtw mbw b_p15 btn-big">
				<button type="submit" name="editsubmit_btn" id="editsubmit_btn" value="true" class="touch">{lang submit}</button>
			</div>
		</form>
	</div>
</div>
	...
	<div class="cl b_p15 bz-at-form">
		...
		<!--{loop $_G['cache']['forums'] $fid $forum}-->
			...
			<tr {if $key++%2==0}class="alt"{/if}>
				<th><a href="forum.php?mod=forumdisplay&fid=$forum[fid]" {if $forum['type'] == 'forum'}style="color: rgba(0,0,0,0.5);"{elseif $forum['type'] == 'sub'}style="color: rgba(0,0,0,0.5);"{/if}>$forum[name]</th>
				<!--{loop $perms $perm}-->
					<td>
...
<div class="ug_expirylist cl">
	<!--{hook/spacecp_usergroup_top}-->
	<!--{subtemplate home/spacecp_usergroup_header}-->
	<div class="b_p15 bzbb1">
		<p class="tbmu">
			<span class="y">
			...
			<tbody>
				<!--{loop $expirylist $groupid $group}-->
					<tr class="{echo swapclass('alt')}">
						<td><a href="home.php?mod=spacecp&ac=usergroup&gid=$groupid">$group[grouptitle]</a></td>
						<td>
						...
						<td>
							<!--{if in_array($groupid, $extgroupids) || $groupid == $_G['groupid']}-->
								<!--{if $groupid != $_G['groupid']}-->
									<!--{if !$group[noswitch]}-->
										<a href="home.php?mod=spacecp&ac=usergroup&do=switch&groupid=$groupid&handlekey=switchgrouphk" class="blue">{lang memcp_usergroups_set_main}</a>
									<!--{/if}-->
									<!--{if !$group['maingroup']}-->
										<!--{if $_G['cache']['usergroups'][$groupid]['pubtype'] == 'buy'}-->
											<a href="home.php?mod=spacecp&ac=usergroup&do=buy&groupid=$groupid&handlekey=buygrouphk" class="blue">{lang renew}</a>
										<!--{/if}-->
										<a href="home.php?mod=spacecp&ac=usergroup&do=exit&groupid=$groupid&handlekey=exitgrouphk" class="blue">{lang memcp_usergroups_exit}</a>
									<!--{/if}-->
								<!--{else}-->
									<!--{if $_G['cache']['usergroups'][$groupid]['pubtype'] == 'buy'}-->
										<a href="home.php?mod=spacecp&ac=usergroup&do=buy&groupid=$groupid&handlekey=buygrouphk" class="blue">{lang renew}</a>
								...
								<!--{/if}-->
							<!--{elseif $_G['cache']['usergroups'][$groupid]['pubtype'] == 'free'}-->
								<a href="home.php?mod=spacecp&ac=usergroup&do=buy&groupid=$groupid&handlekey=buygrouphk" class="blue">{lang free_buy}</a>
							<!--{elseif $_G['cache']['usergroups'][$groupid]['pubtype'] == 'buy'}-->
								<a href="home.php?mod=spacecp&ac=usergroup&do=buy&groupid=$groupid&handlekey=buygrouphk" class="blue">{lang memcp_usergroups_buy}</a>
							<!--{/if}-->
						</td>
					</tr>
				<!--{/loop}-->
	...
	<!--{else}-->
		<p class="hm b_p15">{lang memcp_usergroup_unallow}</p>
	<!--{/if}-->
</div>
<!--{hook/spacecp_usergroup_bottom}-->
...
<!--{else}-->
	...
	<div class="else cl">
		...
		<div class="tdats cl bzbb1 b_p15">
			<div class="z one bz-at-form">
				<table cellpadding="0" cellspacing="0">
			...
			<!--{if $group}-->
				...
				<div class="z three bz-at-form">
					<table cellpadding="0" cellspacing="0">
						<tr>
							<th class="c0"><h4>$tlang - $currentgrouptitle</h4></th>
						</tr>
						<tr>
							<th class="alt h">
								...
								<!--{if isset($publicgroup[$group['groupid']]) && $group['groupid'] != $_G['groupid'] && $publicgroup[$group['groupid']]['allowsetmain']}-->
									<a href="home.php?mod=spacecp&ac=usergroup&do=switch&groupid=$group['groupid']&gid=$_GET[gid]&handlekey=switchgrouphk" class="blue">{lang memcp_usergroups_set_main}</a>
								<!--{/if}-->
								<!--{if in_array($group['groupid'], $extgroupids) && $switchmaingroup && $group['grouptype'] == 'special' && $group['groupid'] != $_G['groupid']}-->
									<!--{if $_G['cache']['usergroups'][$group['groupid']]['pubtype'] == 'buy'}-->
										<a href="home.php?mod=spacecp&ac=usergroup&do=buy&groupid=$group['groupid']&gid=$_GET[gid]&handlekey=buygrouphk" class="blue">{lang renew}</a>
									<!--{/if}-->
									<a href="home.php?mod=spacecp&ac=usergroup&do=exit&groupid=$group['groupid']&gid=$_GET[gid]&handlekey=exitgrouphk" class="blue">{lang memcp_usergroups_exit}</a>
								<!--{/if}-->
								<!--{if $group['grouptype']=='special' && $group['groupid'] != $_G['groupid'] && array_key_exists($group['groupid'], $publicgroup) && !$publicgroup[$group['groupid']]['allowsetmain']}-->
									<a href="home.php?mod=spacecp&ac=usergroup&do=buy&groupid=$group['groupid']&gid=$_GET[gid]&handlekey=buygrouphk" class="blue">{lang memcp_usergroups_buy}</a>
								<!--{/if}-->
								<!--{if array_key_exists($group['groupid'], $groupterms['ext'])}-->
									<span class="notice"><br>{lang memcp_usergroups_timelimit}: <!--{date($groupterms[ext][$group['groupid']])}--></span>
								<!--{/if}-->
							</th>
						</tr>

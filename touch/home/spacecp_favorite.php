...
<!--{if $_GET['op'] == 'delete'}-->
	<form id="favoriteform_{$favid}" name="favoriteform_{$favid}" method="post" autocomplete="off" action="home.php?mod=spacecp&ac=favorite&op=delete&favid=$favid&type=$_GET[type]&mobile=2">
		...
		<dt><p>{lang delete_favorite_message}</p></dt>
		<dd class="cl">
			<a href="javascript:;" onclick="popup.close();" class="bz_btn_close grey">{lang cancel}</a>
			<a class="bz_btn_confirm"><span><input type="submit" name="deletesubmitbtn" value="{lang determine}" class="formdialog"></span></a>
		</dd>
	</form>
<!--{else}-->
	<form method="post" autocomplete="off" id="favoriteform_{$id}" name="favoriteform_{$id}" action="home.php?mod=spacecp&ac=favorite&type=$type&id=$id&spaceuid=$spaceuid&mobile=2" >
		...
		<dt>
			...
			<textarea id="general_{$id}" name="description" rows="1" class="bz-input" ><!--{if $description}-->$description<!--{else}-->{lang favorite_description_default}<!--{/if}--></textarea>
		</dt>
		<dd class="cl">
			<a href="javascript:;" onclick="popup.close();" class="bz_btn_close grey">{lang cancel}</a>
			<a class="bz_btn_confirm"><span><input type="submit" name="favoritesubmit_btn" id="favoritesubmit_btn"  value="{lang determine}" class="formdialog"></span></a>
		</dd>
	</form>
<!--{/if}-->
</div>

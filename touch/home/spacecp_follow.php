...
<!--{if $op == 'bkname'}-->
	...
	<form method="post" autocomplete="off" id="bknameform_{$_GET[handlekey]}" name="bknameform_{$_GET[handlekey]}" action="home.php?mod=spacecp&ac=follow&op=bkname&fuid=$followuser['followuid']" {if $_G[inajax]}onsubmit="ajaxpost(this.id, 'return_$_GET[handlekey]');"{/if}>
		...
		<div class="tip tip_editbkname cl">
			<div class="cl b_p15 bg bzbb1">
				<a href="javascript:;" onclick="popup.close();" class="delete"><i class="banzhuan_font icon-close grey"></i></a>
				<a href="home.php?mod=space&uid=$followuser['followuid']&do=profile" class="avatar"><!--{avatar($followuser['followuid'],middle)}--></a>
				<p class="mbm grey" id="return_$_GET[handlekey]">{lang follow_for}<span class="blue"> $followuser['fusername'] </span>{lang follow_add_bkname}</p>
			</div>
			<div class="detail b_p15 cl">
				<input type="text" name="bkname" value="$followuser['bkname']" size="35" class="bz-input" placeholder="{lang follow_editnote}" />
				<div class="mtw btn-big">
					<button type="submit" name="editsubmit_btn" id="editsubmit_btn" value="true" class="touch">{lang save}</button>
				</div>
			</div>
		</div>
	</form>
